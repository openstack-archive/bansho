...
angular.module('bansho.datasource', ['bansho.surveil'])
    ...
    .service('hostSource', ['$interval', 'surveilStatus', function ($interval, surveilStatus) {
        var hosts = {},
            listeners = [];
        ...
        function getHost(hostname) {
            surveilStatus.getHost(hostname).then(function (data) {
                surveilStatus.getService(hostname).then(function(services) {
                    ...
                    hosts[hostname] = data[0];
                    hosts[hostname].host_services = {};
                    angular.forEach(services, function (service) {
                        hosts[hostname].host_services[service.service_service_description];
                    });

                    console.log(hosts)
                    notifyDataChanged(hostname)
                });
            });
        }

        function notifyDataChanged(hostname) {
            angular.forEach(listeners[hostname], function (callback) {
                callback(hosts[hostname]);
            });
        }

        return {
            setHostRefresh: function (hostname, refresh) {
                getHost(hostname);
                $interval(getHost(hostname), refresh)
            },
            registerDataChanged: function (hostname, callback) {
                if (!listeners[hostname]) {
                    listeners[hostname] = [];
                }

                listeners[hostname].push(callback);
            },
