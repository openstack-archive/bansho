                    ...
                    $scope.sendRecheck = function () {
                        angular.forEach($scope.options.attributes.tableId, function (tableId) {
                            datasource.forEachCheckedEntry(tableId, function (entry) {
                                surveilActions.recheck(entry.host_name, entry.service_description).then(function (data) {
                                        notifications.push('success', 'Recheck', 'Scheduled recheck for ' + entry.host_name + ' ' + entry.service_description);
                                    },
                                    function (error) {
                                        notifications.push('error', 'Recheck', 'Could not schedule recheck for ' + entry.host_name + ' ' + entry.service_description);
                                    });
                            });
