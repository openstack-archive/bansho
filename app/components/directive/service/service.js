            ...
            controller: ['$scope', 'pageParams', 'surveilStatus', 'iframeUrl',
                function ($scope, pageParams, surveilStatus, iframeUrl) {
                    ...
                    surveilStatus.getService(hostname, serviceDescription).then(function (data) {
                        ...
                        surveilStatus.getServiceMetricNames($scope.param.service.host_host_name, $scope.param.service.service_service_description).then(function(metric_names) {
                            $scope.param.service.iframeUrls = {};
                            angular.forEach(metric_names, function (metric) {
                                var metricName = metric.metric_name.substr(7);
                                $scope.param.service.iframeUrls[metricName] = iframeUrl.getIFrameUrl(metric.metric_name, $scope.param.service.host_host_name, $scope.param.service.service_service_description);
                            });
                        });
                    });
