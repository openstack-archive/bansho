...
angular.module('bansho.table', ['bansho.surveil',
    ...
    .service('tables', ['$filter', 'surveilStatus', 'tableGlobalConfig',
        ...
        return {
            ...
            getNoRepeatCell: function (tableId) {
                return config[tableId].noRepeatCell;
            },
            getWrappableCell: function (tableId) {
                return config[tableId].wrappableCell;
            }
        };
    }])
    ...
    .filter('wrappableStyle', ['tables', 'tableGlobalConfig', function (tables, tableGlobalConfig) {
        return function (input, scope) {
            var last = '',
                ...
                class_name = ['', ''],
                i,
                fieldToWrap = tableGlobalConfig.cellWrappableField[tables.getNoRepeatCell(scope.tableId)];

            if (fieldToWrap === undefined) {
                return input;
            }

            if (tables.getWrappableCell(scope.tableId)) {
    ...
    .filter('noRepeat', ['tables', 'tableGlobalConfig', function (tables, tableGlobalConfig) {
        return function (items, scope) {
            var newItems = [],
                previous,
                fieldToCompare = tableGlobalConfig.cellWrappableField[tables.getNoRepeatCell(scope.tableId)],
                newAttr = tables.getNoRepeatCell(scope.tableId) + "_additionnalClass";
